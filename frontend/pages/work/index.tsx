...
const WorkCard = (project: ProjectCardData, index: number) => {
  const FillSkills = () => {
    return project.skills.map((skill: string, index: number) => (
      <div className="skill project-skill" key={index}>
        <p className="body-small" style={{ fontSize: 16 }}>
          {skill}
        </p>
      </div>
    ));
  ...
  return (
    ...
    >
      <img
        className="project-photo work-photo"
        src={project.photo}
        loading="lazy"
        alt="project photo"
      />
      <div className="content-block">
        ...
        <div className="skills-block">{FillSkills()}</div>
      </div>
    </a>
  );
};
...
const WorkCards = (
  ...
  return projects.map((project: ProjectCardData, index: number) =>
    isAll ? (
      WorkCard(project, index)
    ) : isWeb ? (
      project.type === "Web" ? (
        WorkCard(project, index)
      ) : (
...
const Work = () => {
  ...
  const filters: string[] = ["All", "Web", "Software", "Game"];

  const states: boolean[] = [isAll, isWeb, isSoftware, isGame];
  ...
  const setFilters = () => {
    ...
    return (
      <div className="projects-filter">
        {filters.map((filter: string, index: number) => Filter(filter, index))}
      </div>
    );
  };
